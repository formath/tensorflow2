...
class MutableHashTable(LookupInterface):
  ...
  def __init__(self,
    ...
    self._table_ref = self._resource_handle
    if checkpoint:
...
class PartitionedMutableHashTable(object):
  ...
  def __init__(self,
    ...
    for i in range(shard_num):
      mutable_hash_table = MutableHashTable(key_dtype=key_dtype,
                               value_dtype=value_dtype,
                               default_value=default_value,
                               shared_name=shared_name,
                               name='%s_part%d' % (name, i),
                               checkpoint=checkpoint)
      self._table_ref_list.append(mutable_hash_table)
      if trainable:
        ops.add_to_collections([ops.GraphKeys.TRAINABLE_RESOURCE_VARIABLES], mutable_hash_table)
